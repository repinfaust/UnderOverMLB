...
function getPitchingAdjustment(gameData: GameData): { adjustment: number; factors: string[] } {
  ...
  if (eliteCount === 2) {
    ...
    adjustment -= 1.1; // One elite pitcher = stronger suppression
  ...
  // Bullpen fatigue (from game date - approximated)
...
function getWeatherAdjustment(gameData: GameData): { adjustment: number; factors: string[] } {
  ...
  // Temperature effects (enhanced hot weather boost)
  if (weather.temp_f > 95) {
    adjustment += 0.5; // Extreme heat - increased impact
    factors.push('extreme_heat');
  } else if (weather.temp_f > 90) {
    adjustment += 0.3; // Very hot weather - new bracket
    factors.push('very_hot_weather');
  } else if (weather.temp_f > 85) {
    adjustment += 0.2; // Hot weather - increased from 0.1
 ...
 */
function calculateConfidence(components: ComponentBreakdown): number {
...
export function predictGameTotal(gameData: GameData): ComponentBreakdown {
  ...
  breakdown.confidence = calculateConfidence(breakdown);
